...
GetHint.get("/get-hint", async (req, res) => {
  ...
  if (nextRoundNumber == 1) {
    ...
    const exists = resultData.taskHistory.some(
    ...
    if (
      !questionData ||
      !questionData.rounds ||
      questionData.rounds.length === 0
    ) {
      return res
        .status(404)
        .json({ message: "No questions found for Round 1." });
    }
  ...
  if (nextRoundNumber == 2) {
    ...
    if (
      ...
      return res
        .status(404)
        .json({ message: "No questions found for Round 2." });
    }
  ...
  if (nextRoundNumber == 3) {
    ...
    if (
      ...
      return res
        .status(404)
        .json({ message: "No questions found for Round 3." });
    }
